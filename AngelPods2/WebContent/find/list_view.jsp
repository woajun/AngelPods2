...
<title>엔젤팟 : 습득물 조회 게시판</title>
    ...
    <link href="../find/find.css/list_view.css?v=<%=System.currentTimeMillis() %>" rel="stylesheet">

  <style>
    
#curPositionBtn{
  position:fixed; right: 4vh; top:80px;
}

#map{
  width:100%;height:100vh; position: fixed; padding-left: 480px;
}
...
<body>
  ...
  <button id="curPositionBtn" type="button" class="btn btn-primary" onclick = "curPositionBtn()">
    ...
    <!-- 글쓰기 버튼 -->
    <button id="btn_write" type="button" onclick = "javascript:window.location='f_write_view.do'"
...
for (let i = 0; i< lat.length;i++){
     ...
     '        <div><button class="btn btn-sm btn-outline-secondary" onclick="location.href=\'f_content_view.do?fbNum='+fbNum[i].value+'\'" > 게시물보기 </button></div>' + 
     '      </div>' + 
     '    </div>' +    
     '</div>';
